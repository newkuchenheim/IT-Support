...
function init() {
	...
	function sendEmail() {
		...
		var _accOwner = document.getElementById("accOwner").value;
		var _createdBy = document.getElementById("createdBy").value;
		//var _ccEmail = document.getElementById("cc_email").value;
		// create email parts
		var subject = "Änderungsmitteilung Bankverbindung ";
		var ccPart = "";
		//if (_ccEmail !== null && _ccEmail !== "") ccPart = "&cc=" + _ccEmail;
		var body;
		if (_prename !== "" && _name !== "" && _location_text !== "" && _dateValidFrom !== "" && _iban !== "" && _createdBy !== "") {
	...
	var steps = [
		{
			title: "Anleitung Änderungsmitteilung - Bankverbindung",
		...
		},
		{
			...
			title: "3. Schritt",
			content: "Geben Sie den das \"Gültig ab\" Datum an.",
			placement: "bottom",
			btnNext: { text: wtConfig.NextText, backgroundColor: wtConfig.NextBgColor, textColor: wtConfig.NextTextColor },
			btnBack: { text: wtConfig.BackText, backgroundColor: wtConfig.BackBgColor, textColor: wtConfig.BackTextColor },
			onNext: function () {
		...
		},
		{
			element: "#step_iban",
			title: "4. Schritt",
			content: "Geben Sie die neue IBAN an.",
			placement: "bottom",
			btnNext: { text: wtConfig.NextText, backgroundColor: wtConfig.NextBgColor, textColor: wtConfig.NextTextColor },
			btnBack: { text: wtConfig.BackText, backgroundColor: wtConfig.BackBgColor, textColor: wtConfig.BackTextColor },
			onNext: function () {
				var costunit = document.getElementById("iban").value;
		...
		{
			element: "#step_accowner",
			title: "5. Schritt",
			content: "Geben Sie den Kontoinhaber an, falls dieser abweicht.",
			placement: "bottom",
			btnNext: { text: wtConfig.NextText, backgroundColor: wtConfig.NextBgColor, textColor: wtConfig.NextTextColor },
			btnBack: { text: wtConfig.BackText, backgroundColor: wtConfig.BackBgColor, textColor: wtConfig.BackTextColor }
		},
